import {
  ...
  insertUser,
  updateUser, updateUserAfterUpload
} from './user.query'
import {
  FieldTypeUser,
  FieldTypeUserJWT,
...
import { generateOTPCode } from '../../utils/utilFunctions'
import { ObjectId } from 'mongoose'
import { UploadPath } from '../../utils/enum'
import { fileRemover } from '../../middleware/fileHandler'
...
const conFetchUserCredentials = async (req, res, next) => {
  const { userId } = req.loggedInUser as FieldTypeUserJWT
  try {
    const userCredential = await fetchUserCredential(userId)
    res.status(userCredential ? responseCode.OK : responseCode.OK)
      .send(makeSuccessObject<FieldTypeUser>(userCredential, userResponse.success.USER_CREDENTIALS))
  } catch (_err) {
    next({ message: userResponse.error.USER_CREDENTIALS, status: responseCode.UNAUTHORIZED })
  }
}
...
const conLoginUser = (req, res, next) => {
  ...
  fetchUserByKeyValue({ email })
    .then(async (user) => {
      ...
      if (!user.isVerified) {
        return next({ message: userResponse.error.UNVERIFIED_LOGIN, status: responseCode.UNAUTHORIZED })
      }
...
const conFetchUserByCategory = async (req, res, next) => {
  ...
  if (category !== (UserCategory.RENTER || UserCategory.TENANT)) {
...
const conUpdateUserAfterUpload = (req, res, next) => {
  if (!req.file) {
    return next({ message: uploadResponse.ERROR('image'), status: responseCode.INTERNAL_SERVER })
  }
  const filePath = `/image/${UploadPath.USER}/${req.file.filename}`
  ...
  fetchUserById(userId)
    .then((user) => {
      ...
      if (user.image) {
        fileRemover(user.image)
      }
      updateUserAfterUpload(userId, filePath)
        .then((user) => {
          res.status(responseCode.ACCEPTED).send(makeSuccessObject<FieldTypeUserMain>(user, uploadResponse.SUCCESS('Image')))
        })
        .catch(() => {
          next({ message: uploadResponse.UPDATE, status: responseCode.BAD_REQUEST })
        })
    })
    .catch(() => {
      next({})
