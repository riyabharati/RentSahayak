...
export default function FormComponent() {
  ...
  const res=await apiUpdateBookingStatus(bookingData._id,value)
    if(res.data.success){
      setUpdateStatus(bookingData.status==="accepted")
  ...
  const handleDecline=async(value)=>{
    const res=await apiUpdateBookingStatus({...value})
    if(res.data.success){
      setUpdateStatus(bookingData.status==="declined")
            ...
            <Col className="mr-5">
              ...
              <div >
                ...
                {bookingData.conversation.map((conv)=>(
                  <div>
                    <h5>{conv.userId.profile.fullName}<span>{new Date(conv.date).toDateString()}</span></h5>
                    <p>{conv.message}</p>
                  </div>
                ))}
              ...
              <Button type="submit" onClick={handleResolve} value={status} className='w-20 mr-4'>Accept</Button>
              <Button type="submit" onClick={handleDecline} value={status} className='w-20 bg-danger'>Decline</Button>
            </Col>
          </React.Fragment>
        </div>
        )
